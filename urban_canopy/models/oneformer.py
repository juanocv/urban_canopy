...
def class_space_for_model(model_name: str) -> str:
    """Infer the class space a checkpoint predicts from its name."""
    lowered = model_name.lower()
    for hint, space in _CLASS_SPACE_HINTS:
        if hint in lowered:
            return space
    raise ValueError(
        f"Cannot infer the class space of OneFormer checkpoint {model_name!r}. "
        "Pass an explicit taxonomy so the tree classes are unambiguous."
    )
